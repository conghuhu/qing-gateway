 ...
 * @returns
 */
export function getLoadBalance(): Promise<String[]> {
 ...
 * @returns
 */
export async function queryInitStatus() {
  return await request.get('/sys/queryInitStatus');
}
/**
 * 系统初始化过
 * @param param
 ...
 * @returns
 */
export async function getCurrentConfig(): Promise<Infoconfig> {
  return await request.get('/sys/getCurrentConfig');
}
/**
 * 重置系统初始化信息
 * @param param
